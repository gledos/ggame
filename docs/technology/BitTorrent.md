...
+   <ruby>Bram Cohen<rp>(</rp><rt>布莱姆·科亨</rt><rp>)</rp></ruby>
...
+   Point to point
    ...
    点对点，常见于 Client Server (CS) 架构的网络，比如<ruby>万维网<rp>(</rp><rt>WWW</rt><rp>)</rp></ruby>主要就是这样的结构。
...
原本的 urn 的命名空间从 btih 变成了 btmh，含义从 BitTorrent info hash 变成了 BitTorrent multi hash，表示的是「哈希树」，然后使用 SHA-2 的 SHA-256 算法取代了 SHA-1。并且支持创建和使用混合版本的种子，下面是一些示例，分别是 v1, v2 和混合。[^lbb2]

[^lbb2]: arvid, 《[BitTorrent v2](https://web.archive.org/web/20221126212909/https://blog.libtorrent.org/2020/09/bittorrent-v2/)》, libtorrent blog, 2020-09-07. (参照 2022-12-01).
